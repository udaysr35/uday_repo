...
# The Groq/OpenAI API key will be loaded from Codespaces Secrets


# Load Model
llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0)       # <--- if using Groq model
# llm = ChatOpenAI(model_name="gpt-4o-mini", temperature=0)           # <--- if using OpenAI model
...
If user asks for unique companies symbols present in table then do not limit the output.
...
Output in JSON format with below key:
- sql_query
...
# Create Class for Custom Object using Pydantic
from pydantic import BaseModel, Field

class SQLQuery(BaseModel):
    sql_query: str = Field(description="The SQL query generated by the model")


# Set up a parser + inject instructions into the prompt template.
from langchain_core.output_parsers import PydanticOutputParser
pydantic_sql_parser = PydanticOutputParser(pydantic_object=SQLQuery)


# SQL Query Generation Chain
sql_chain = (PROMPT1
             | llm
             | pydantic_sql_parser       # to get output in a more usable format
             )
...
Generate and return the python code only, and NO Additional Text.
"""

PROMPT2 = PromptTemplate(input_variables=["request", "sql_query"], template=template2)
...
sql_code_chain = {
    ...
    "sql_query": lambda x: sql_chain.invoke(x["request"]).sql_query
    } | code_chain
